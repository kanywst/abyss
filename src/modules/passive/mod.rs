...
use crate::models::{
    AlienVaultData, ArchiveRecord, GeoIntel, GitHubRecord, ShodanData, SslIntel, UrlScanRecord,
};
use reqwest::Client;
...
pub async fn scan_alienvault(target: &str, client: &Client) -> Result<AlienVaultData> {
    let url = format!(
        "https://otx.alienvault.com/otxapi/indicators/domain/{}/general",
        target
    );
    ...
    if let Ok(resp) = client.get(&url).send().await {
        if let Ok(json) = resp.json::<Value>().await {
            ...
            data.related_tags.sort();
            data.related_tags.dedup();
        }
    }
    Ok(data)
}
...
pub async fn scan_urlscan(target: &str, client: &Client) -> Result<Vec<UrlScanRecord>> {
    let url = format!(
        "https://urlscan.io/api/v1/search/?q=domain:{}&size=20",
        target
    );
    let mut records = Vec::new();
    if let Ok(resp) = client.get(&url).send().await {
            ...
            if let Some(hostnames) = json["hostnames"].as_array() {
                for h in hostnames {
                    if let Some(s) = h.as_str() {
                        data.hostnames.push(s.to_string());
                    }
                }
            }
